export default async function handler(req, res) {
  ...
  try {
    ...
    const { prompt = '', messages = [], model = 'gpt-4o-mini', mode = 'chat', temperature = 0.7, systemPrompt } = req.body || {};
    ...
    if (inputMessages.length === 0 && String(prompt).trim()) {
      inputMessages = [{ role: 'user', content: String(prompt).trim() }];
    }

    if (inputMessages.length === 0) {
      return res.status(400).json({
        error: 'Prompt/messages kosong. Frontend harus mengirim prompt atau messages.',
